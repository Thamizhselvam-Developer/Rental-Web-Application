...
const baseServiceData = async (data) => {
    try {
        ...
        const result = await AdminRoleModel.baseModelAdminRole(nanoid, data.name, data.email, hashedPassword, Data);
...
const baseServiceUpdateData = async (data) => {
    try {
        ...
        const result = await AdminRoleModel.baseModelUpdate(data.id, data.name, data.email);
        const res = await AdminRoleModel.baseModelUpdateRoleData(data.adminRoleId, data.adminroleName);
        return { result, res };
    } catch (error) {
...
module.exports = { baseServiceData, baseServiceGetData,checkIfRoleExists, baseServiceUpdateData};
